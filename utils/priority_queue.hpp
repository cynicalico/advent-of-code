...
public:
    ...
    bool push_or_update(std::size_t key, std::size_t priority, T value, bool only_if_higher = false) {
        if (key < key_to_heap_pos_.size() && key_to_heap_pos_[key] != HEAP_POS_NONE)
            return update(key, priority, only_if_higher);

        return push(key, priority, value);
    }
    ...
    bool push_or_update(std::size_t priority, T value, bool only_if_higher = false) {
        return push_or_update(size(), priority, value);
    }

    bool push(std::size_t priority, T value) { return push(size(), priority, value); }
