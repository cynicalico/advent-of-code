...
std::generator<Vec2<T>> moore_neighborhood_seq(const Vec2<T> &p) {
    ...
    for (const auto &d : OFFSETS) co_yield p + d;
}
...
std::generator<Vec2<T>> grid_seq(T x_min, T x_max, T y_min, T y_max) {
    for (T y = y_min; y <= y_max; ++y)
        for (T x = x_min; x <= x_max; ++x) co_yield Vec2<T>{x, y};
}
...
template <typename T>
auto sum(T &&t) {
    return std::ranges::fold_left_first(std::forward<T>(t), std::plus{}).value();
}

template <typename T>
auto prod(T &&t) {
    return std::ranges::fold_left_first(std::forward<T>(t), std::multiplies{}).value();
}
} // namespace utils
